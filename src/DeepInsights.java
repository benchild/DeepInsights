...
public class DeepInsights
{
	...
	static double[] min = new double[maxVarNum], max = new double[maxVarNum], maxAbs = new double[maxVarNum], stu = new double[maxLineNum];
	...
	public static void preprocess()
	{
		for (int i=0; i<varNum; i++)
		{
			...
			maxAbs[i] = Math.max(Math.abs(min[i]), Math.abs(max[i]));
		}
	}     
	...
	public static void search_nonlinear(int i, int pos)
	{
		...
		if (j != unique)
		{
			...
			if (maxAbs[j]>1)
			{
				...
				for (int k=j+1; k<varNum; k++) if (maxAbs[k]>1)
				{
	...
	{
		if (list.get(0).x == 11 && list.get(1).x == 7 && list.get(1).type == "rec")
		{
			int k = 10;
			k = 20;
		}
		...
		alpha = 1/maxAbs[list.get(0).x]/maxAbs[list.get(0).x];
		...
		for (int k=0; k<100; k++)
		{
			...
			for (int i=0; i<slineNum; i++)
			{
				...
				temp = ma[0][i]-temp;  
				error += temp*temp;
				if (Math.abs(temp/ma[0][i])>0.1) error_time++;
			}
			if (k == 0) first_error = error;
			if (error > 2*old_error) alpha = alpha/10;
			else if (old_error>error && (old_error-error)/error<0.1) alpha = alpha*10;
			old_error = error;
			for (int i=0; i<slineNum; i++)
			{
				temp = par[0];
		...
		for (int k=0; k<1000; k++)
		{
			...
			if (error > 2*old_error) alpha = alpha/10;
			else if (k<500 && old_error>error && (old_error-error)/error<0.1) alpha = alpha*10;
			old_error = error;
			for (int i=0; i<lineNum; i++)
			{
				temp = par[0];
		...
		*/
		...
		if (iflag < lineNum*0.2+2) {
			...
			*/
			...
			result.add(new nonLinearInv(list, par));
	...
	public static void descent(int size) 
	{
		...
		for (int k=0; k<100; k++)
		{
			...
			if (k == 0) first_error = error;
			if (error > 2*old_error) alpha = alpha/10;
			else if (k<50 && old_error>error && (old_error-error)/error<0.1) alpha = alpha*10;
			old_error = error;
			for (int i=0; i<slineNum; i++)
			{
				temp = par[0];
	...
	public static void print(long time) throws IOException
	{
		...
		int total = 0;
		for (int i=0; i<result.size(); i++)
			if (use[i] && result.get(i).error<lineNum*0.1) {
			total++;
			writer.write(result.get(i).toString()+'\n');
	...
	public static void test(int p, int q) throws IOException 
	{
		BufferedWriter writer = new BufferedWriter(new FileWriter("input.smt2"));
		for (int i=0; i<varNum; i++) 
			writer.write("(declare-const "+"a"+i+" Real)\n");

		String s = result.get(p).toPrefix();
		if (s != "") writer.write("(assert ("+s+"))\n");
		if (result.get(p) instanceof nonLinearInv) 
		{
			String type = ((nonLinearInv)(result.get(p))).list.get(1).type;
			if (type == "rec")
				writer.write("(assert (not (= a"+((nonLinearInv)(result.get(p))).list.get(1).x+" 0)))\n");
			else if (type == "div")
				writer.write("(assert (not (= a"+((nonLinearInv)(result.get(p))).list.get(1).y+" 0)))\n");
		}
		s = result.get(q).toPrefix();
		if (s != "") writer.write("(assert (not ("+s+")))\n");
		if (result.get(q) instanceof nonLinearInv) 
		{
			String type = ((nonLinearInv)(result.get(q))).list.get(1).type;
			if (type == "rec")
				writer.write("(assert (not (= a"+((nonLinearInv)(result.get(q))).list.get(1).x+" 0)))\n");
			else if (type == "div")
				writer.write("(assert (not (= a"+((nonLinearInv)(result.get(q))).list.get(1).y+" 0)))\n");
		}
		writer.write("(check-sat)\n");
        writer.close();
	}
